...
│   │   ├── ddl/
│   │   └── dml/
│   ├── evidencias consulta y transacciones/
│   │   ├── ss consultas/
│   │   └── ss transacciones/
│   └── acceso remoto/
│       ├── conexion/
...
## 5. Conexion a la base de datos remota|

Para conectarse desde la consola:

```bash
psql -h [HOST] -p 5432 -U [USUARIO] -d logitrack
```

O desde pgAdmin, crear una conexion con los parametros anteriores.

## 6. Consideraciones para la restauracion
