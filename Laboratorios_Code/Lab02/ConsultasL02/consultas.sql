...
*/
SET search_path TO logitrack; -- CORRECCIÓN: sin esta línea las tablas no se encuentran fuera del esquema logitrack
...
-- CORRECCIÓN: se reemplazó "RETURNING id_orden \gset new_order_" + ":new_order_id" (solo psql)
-- por currval(), que funciona tanto en psql como en pgAdmin4: devuelve el último id generado
-- por la secuencia de la tabla ordenes dentro de la sesión actual.
