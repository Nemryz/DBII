...
-- la relación entre Bodega y Ubicaciones es de uno a muchos, una bodega tiene múltiples ubicaciones (regla de negocio), pero cada ubicación pertenece a una sola bodega.
...
-- la tabla Detalle_Ordenes es la resolución formal de la relación muchos a muchos entre Ordenes y Productos (N:N), porque, una orden contiene muchos productos y un producto puede aparecer en muchas órdenes.
...
-- el UNIQUE (id_orden, id_producto) garantiza que un mismo producto no se repita como línea dentro de una misma orden, y obliga a sumar cantidades en una sola línea en lugar de duplicarlas.
-- el CHECK (cantidad > 0) junto con la regla de negocio "cada orden debe tener al menos un producto asociado" asegura que ninguna línea de detalle sea vacía, en sí, por eso el mínimo de una línea por orden se controla a nivel de aplicación/transacción, ya que SQL no permite imponer un mínimo de filas hijas por fila padre.
-- el precio_unitario se guarda aquí (y no solo en Productos) como una instantánea del precio al momento de la orden, si el precio del producto cambia luego, las órdenes históricas conservan el valor con el que fueron registradas, lo que es clave para la trazabilidad.
...
-- la relación entre Ordenes y Envios se adopta como 1:N, fiel al trazo del diagrama ER del laboratorio, dado que en general, una orden puede despacharse en uno o más envíos (por ejemplo, despachos parciales o reenvíos), y cada envío pertenece a una única orden.
...
-- fecha_despacho con TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP registra automáticamente cuándo sale el despacho.
-- fecha_entrega es opcional (puede ser NULL) porque no todos los envíos tienen una fecha de entrega definida al momento de su registro, solo la tendrá aquel envío que ya haya sido entregado.
-- el CHECK sobre estado restringe el ciclo de vida del envío a un catálogo cerrado: programado, en tránsito, entregado o rechazado.